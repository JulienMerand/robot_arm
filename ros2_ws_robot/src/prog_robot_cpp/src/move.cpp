...
class Move : public rclcpp::Node
{
  ...
  private:
    void pose_callback(const std_msgs::msg::Float32MultiArray msg);
    
    Eigen::Matrix4d get_transformation_matrix(geometry_msgs::msg::TransformStamped transform);
...
Move::Move() : Node("move", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true)),
               ...
               tf_buffer(std::shared_ptr<rclcpp::Node>(std::move(this))->get_clock()),
...
Eigen::Matrix4d Move::get_transformation_matrix(geometry_msgs::msg::TransformStamped transform)
{
    tf2::Matrix3x3 rot;
